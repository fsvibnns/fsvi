...
import sys
root_folder = '/scratch-ssd/ANON/deployment/testing/projects/function-space-variational-inference'
sys.path.insert(0, root_folder)

from fsvi.models.networks import MLP
...
def make_sac_networks(
  ...
  _init_fn, _apply_fn = policy_module.forward
  params_init, state = _init_fn(rng_key, dummy_obs, rng_key, True, True)
  init_fn = lambda key: _init_fn(key, dummy_obs, key, True, True)[0]
  apply_fn = lambda policy_params, obs, rng_key: _apply_fn(policy_params, state, rng_key, obs, rng_key, True, True)[0]
...
def train(
  ...
  def do_one_step_eval(carry, unused_target_t):
    ...
    logits = policy_model.apply(policy_params, obs, key)
  ...
  def alpha_loss(alpha: jnp.ndarray, policy_params: Params,
    ...
    dist_params = policy_model.apply(policy_params, transitions.o_tm1, key)
  ...
  def critic_loss(q_params: Params, policy_params: Params,
    ...
    q_old_action = value_model.apply(q_params, transitions.o_tm1,
                                     transitions.a_tm1)
    next_dist_params = policy_model.apply(policy_params, transitions.o_t, key)
    ...
    # reg_params = [p for ((mod_name, _), p) in tree.flatten_with_path(q_params)]
    # reg = 1e-5 * jnp.square(optimizers.l2_norm(reg_params))

    q_loss = 0.5 * jnp.mean(jnp.square(q_error)) #+ reg
    return q_loss
  ...
  def collect_data(training_state: TrainingState, state):
    ...
    normalized_obs = obs_normalizer_apply_fn(training_state.normalizer_params,
                                             state.obs)
    logits = policy_model.apply(training_state.policy_params, normalized_obs, key)
...
def make_params_and_inference_fn(observation_size, action_size,
  ...
  def inference_fn(params, obs, key):
    ...
    key_params, rng = jax.random.split(key)
    logits = policy_model.apply(policy_params, obs, key_params)
