...
def linearize(f: Callable[..., PyTree], params: PyTree) -> Callable[..., PyTree]:
...
def taylor_expand(
    f: Callable[..., PyTree], params: PyTree, degree: int
) -> Callable[..., PyTree]:
...
def empirical_kernel_fn(
  ...
  """
    ...
    kernel_fns = {
        ...
        "ntk": empirical_ntk_fn(
            **kwargs, vmap_axes=vmap_axes, implementation=implementation
        ),
    }
...
def empirical_nngp_fn(
    ...
    [NTTree[np.ndarray], Optional[NTTree[np.ndarray]], PyTree], NTTree[np.ndarray]
]:
  ...
  """
    ...
    def nngp_fn(
        x1: np.ndarray, x2: Optional[np.ndarray], params: PyTree, **apply_fn_kwargs
    ) -> np.ndarray:
...
def empirical_ntk_fn(
  ...
  """
    kwargs = dict(
        fn=f, trace_axes=trace_axes, diagonal_axes=diagonal_axes, vmap_axes=vmap_axes
    )
    ...
    # MOD
    if implementation == 3:
        return _empirical_implicit_ntk_bnn_fn(**kwargs)
...
def _empirical_implicit_ntk_fn(
    f: ApplyFn,
    trace_axes: Axes = (-1,),
    diagonal_axes: Axes = (),
    vmap_axes: VMapAxes = None,
    ...
    """
        ...
        args2 = (
            kwargs1[k] if k in kw_axes and kwargs2[k] is None else kwargs2[k]
            for k in keys
        )
        ...
        return _trace_and_diagonal(
            get_ntk(x1, x2, *args1, *args2), trace_axes, diagonal_axes
        )

    return ntk_fn


def _empirical_implicit_ntk_bnn_fn(
    fn: ApplyFn,
    trace_axes: Axes = (-1,),
    diagonal_axes: Axes = (),
    vmap_axes: VMapAxes = None,
    ...
    Returns:
      ...
      All other axes are present twice.
    """
        f = partial(fn, inputs=x1)  # predict_fn is a function of params

        kwargs1, kwargs2 = utils.split_kwargs(apply_fn_kwargs, x1, x2)
        fx1 = eval_shape(fn, params, x1, **kwargs1)
        x_axis, fx_axis, kw_axes = _canonicalize_axes(vmap_axes, x1, fx1, **kwargs1)
        ...
        def get_ntk(x1, x2, *args):
            ...
            def delta_vjp_jvp(delta):
                def delta_vjp(delta):
                    vjp_tp = jax.vjp(f, params)[1](delta)
                    Sigma_flat, _ = tree_flatten(sigma)
                    vjp_tp_flat, vjp_tp_flat_tree = tree_flatten(vjp_tp)
                    vv = []
                    for s, v in zip(Sigma_flat, vjp_tp_flat):
                        vv.append(s * v)
                    return tree_unflatten(vjp_tp_flat_tree, vv)

                return jax.jvp(f, (params,), delta_vjp(delta))[1]

            fx1, fx2 = eval_shape(f, params), eval_shape(f, params)
            eye = _std_basis(fx1)
            ntk = vmap(linear_transpose(delta_vjp_jvp, fx2))(eye)
            ntk = tree_map(lambda fx12: _unravel_array_into_pytree(fx1, 0, fx12), ntk)
            ntk = _diagonal(ntk, fx1)
...
def _empirical_direct_ntk_fn(
    ...
    def sum_and_contract(fx, j1, j2, sigma):
        ndim = fx.ndim
        ...
        Sigma_flat, _ = tree_flatten(sigma)
        j1_flat, j1_flat_tree = tree_flatten(j1)
        vv = []
        for s, v in zip(Sigma_flat, j1_flat):
            vv.append(s * v)
        j1_sigma = tree_unflatten(j1_flat_tree, vv)
        # j1_sigma = tree_multimap(operator.mul, j1, sigma)
        return tree_reduce(operator.add, tree_multimap(contract, j1_sigma, j2))
    ...
    """
        ...
        ntk = sum_and_contract(fx1, j1, j2, sigma)
        return ntk

    return ntk_fn


def _empirical_direct_ntk_bnn_fn(
    f: ApplyFn,
    trace_axes: Axes = (-1,),
    diagonal_axes: Axes = (),
    vmap_axes: VMapAxes = None,
...
def _trace_and_diagonal(
    ntk: np.ndarray, trace_axes: Axes, diagonal_axes: Axes
) -> np.ndarray:
