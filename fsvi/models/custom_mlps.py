...
class ResNet(MLP):
    def __init__(self, *args, **kwargs):
        super().__init__(network_type="resnet", *args, **kwargs)
    ...
    def __call__(self, inputs, rng_key, stochastic, is_training):
        ...
        # FIRST LAYER
        out = self.layers[0](out, kh.next_key(), stochastic)
        ...
        for l in range(1,len(self.layers)-2):
            ...
            if self.batch_norm_layers[l] != None:
                out = self.batch_norm_layers[l](out, is_training=is_training)
            out = out + self.activation_fn(out)

        # PENULTIMATE LAYER
        out = self.activation_fn(self.layers[-2](out, kh.next_key(), stochastic))

        # FINAL LAYER
        out = self.layers[-1](out, kh.next_key(), stochastic)
