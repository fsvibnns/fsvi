...
# TR: added precision hyper
flags.DEFINE_integer('precision', 0.0001, 'Prior precision.')
...
def main(argv):
  ...
  precision = FLAGS.precision  # TR: added
  ...
  # TR: added images_prior argument
  @functools.partial(jax.pmap, axis_name='batch', donate_argnums=(0,))
  def update_fn(opt, lr, images, labels, images_prior, rng):
    """Update step."""
    measurements = {}
    ...
    def loss_fn(params, images, labels, images_prior, precision):
      pred_fn_train = lambda params: model.apply(
          {'params': flax.core.freeze(params)}, images,
          train=True, rngs={'dropout': rng_model_local})[0]
      logits_train, _ = jax.jvp(pred_fn_train, (params,), (params,))[1]

      # TR: compute cross entropy
      llk = getattr(train_utils, config.get('loss', 'sigmoid_xent'))(
          logits=logits_train, labels=labels)

      # TR: compute predictive function on sampled points
      pred_fn_prior = lambda params: model.apply(
          {'params': flax.core.freeze(params)}, images_prior,
          train=True, rngs={'dropout': rng_model_local})[0]
      logits_prior, _ = jax.jvp(pred_fn_prior, (params,), (params,))[1]

      # TR: concatenate training and sampled points and compute norm
      logits_inducing = jnp.concatenate([logits_train, logits_prior], axis=0)
      function_norm = jnp.sum(jnp.square(logits_inducing))

      loss = llk + precision * function_norm
    ...
    l, g = train_utils.accumulate_gradient(
        jax.value_and_grad(loss_fn), opt.target, images, labels, images_prior, precision,  # TR: added images_prior, precision
    ...
    with jax.profiler.TraceAnnotation('train_step', step_num=step, _r=1):
      if not FLAGS.only_eval:
        opt_repl, loss_value, train_loop_rngs, extra_measurements = update_fn(
            ...
            train_batch['image'],
            train_batch['labels'],
            train_batch['image'],  # TODO: TR: this should be separately sampled points; this is just for testing
            rng=train_loop_rngs)
