...
try:
    ...
    print("Python package ml_collections could not be loaded. ViT won't work.")
...
from fsvi.utils.haiku_mod import dense_stochastic_hk, partition_all_params, predicate_batchnorm, predicate_mean
from fsvi.models import resnet
try:
    from fsvi.models import vit
except:
    print("WARNING: ViT could not be imported.")
from fsvi.models import custom_cnns, custom_mlps
from fsvi.utils.utils_linearization import convert_predict_f_only_mean
from fsvi.utils.haiku_mod import gaussian_sample_pytree, partition_params_final_layer_bnn
        ...
        if stochastic_parameters:
            if final_layer_variational:
                if fixed_inner_layers_variational_var:
                    self.inner_layers_stochastic = True
                else:
                    self.inner_layers_stochastic = False
            else:
                self.inner_layers_stochastic = True
        else:
            self.inner_layers_stochastic = False
...
class MLP(Model):
    ...
    def make_forward_fn(self):
        ...
        if not self.resnet:
            ...
            def forward_fn(inputs, rng_key, stochastic, is_training):
                _forward_fn = custom_mlps.ResNet(
                    output_dim=self.output_dim,
                    activation_fn=self.activation_fn,
                    architecture=self.architecture,
                    no_final_layer_bias=self.no_final_layer_bias,
...
class CNN(Model):
    ...
    def make_forward_fn(self):
        ...
        if self.architecture == "four_layers":

            def forward_fn(inputs, rng_key, stochastic, is_training):
                _forward_fn = custom_cnns.FourLayers(
                    output_dim=self.output_dim,
                    activation_fn=self.activation_fn,
                    no_final_layer_bias=self.no_final_layer_bias,
                    stochastic_parameters=self.stochastic_parameters,
        ...
        elif self.architecture == "six_layers":
        ...
        elif self.architecture == "eight_layers":

            def forward_fn(inputs, rng_key, stochastic, is_training):
                _forward_fn = custom_cnns.EightLayers(
                    output_dim=self.output_dim,
                    activation_fn=self.activation_fn,
                    no_final_layer_bias=self.no_final_layer_bias,
                    stochastic_parameters=self.stochastic_parameters,
        ...
        elif self.architecture == "resnet" or self.architecture == "resnet18":
            ...
            def forward_fn():
                ...
                class WrapNet(object):
                    def __init__(self, net):
                        self.init_fn = lambda rng_key, inputs, stochastic, is_training: net.init(rng_key, inputs, train=is_training)
                        self.apply = lambda params, state, rng_key, inputs, rng_key2, stochastic, is_training: (net.apply(params, inputs, train=is_training), {})
