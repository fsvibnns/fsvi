...
from fsvi.utils.haiku_mod import conv2D_stochastic, dense_stochastic_hk
...
class FourLayers(CNN):
    """CNN with 2 conv layers and 2 fc layers."""
    def __init__(self, *args, **kwargs):
        super().__init__(channels=(32, 64), fc_dim=256, *args, **kwargs)
    ...
    def __call__(self, inputs, rng_key, stochastic, is_training):
        ...
        for i in range(2):
            ...
            if self.dropout:
                out = hk.dropout(kh.next_key(), self.dropout_rate, out)
...
class EightLayers(CNN):
    """CNN with 6 conv layers and 2 fc layers."""
    def __init__(self, *args, **kwargs):
        super().__init__(channels=(32, 32, 64, 64, 128, 128), fc_dim=512, *args, **kwargs)
...
class OmniglotCNN(hk.Module):
    ...
    def __init__(
        ...
        super().__init__()
        del dropout, dropout_rate, batch_normalization
        self.activation_fn = activation_fn
        self.max_pool = hk.MaxPool(
            window_shape=(1, 2, 2, 1), strides=(1, 2, 2, 1), padding="VALID"
        )
