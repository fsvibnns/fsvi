...
class Three_layer_Classifier(BaseModel):
    """
    Simple example stochastic variational inference CNN for classification.
    ...
    def __init__(self,
        ...
        self.fc1 = SVI_Linear(64 * 7 * 7,
                              128,
                              initial_rho,
                              initial_mu_std,
                              variational_distribution,
                              prior)
        self.last_layer = SVI_Linear(128,
                                     out_features,
                                     initial_rho,
                                     initial_mu_std,
                                     variational_distribution,
...
class Seven_layer_Classifier(BaseModel):
    """
    Simple example stochastic variational inference CNN for classification.
    ...
    def __init__(self,
                 ...
                 variational_distribution,
                 prior,):
        super(Seven_layer_Classifier, self).__init__()
        self.max_pool = SVIMaxPool2D((2, 2))
    ...
    def forward(self, x):
        ...
        x = self.max_pool(x)
        x = F.relu(self.fifth_conv(x))
        x = F.relu(self.sixth_conv(x))
        x = self.max_pool(x)
